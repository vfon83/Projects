...
def update_plot(E_voltages):    
    ...
    axs[0].set_xlabel('x')
    axs[0].set_ylabel('y')
    axs[0].set_xlim(-0.5, X.max() + 0.5)
    axs[0].set_ylim(-0.5, Y.max() + 0.5)
    axs[0].grid(True)

    # Second subplot (Stream Field)
    stream_plot = axs[1].streamplot(X, Y, Ex_normalized, Ey_normalized, density=0.5, linewidth=3.5, arrowsize=3, color=E_averages, cmap='gist_heat_r', broken_streamlines=False)
